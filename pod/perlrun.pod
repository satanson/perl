...
This removes all layers that do unicode IO, unlikely to be useful on its own in
the global PERLIO environment variable.  You perhaps were thinking of
C<:crlf:bytes> or C<:perlio:bytes>.
